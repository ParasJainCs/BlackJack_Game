	...
	if(bet<=wallet)
		{
			...
			document.getElementById("demo_for_bet_amount_and_wallet_amount").innerHTML="Insufficient balance in wallet!";	
	}
}
...
function bet_doubler(){
	// Need to implement the fact that double can only be clicked if wallet contains doubleable amount!
...
function deal_card_displayer()
{
	...
	if(z==1)
	...
	else if(z==2)	// ace splitting.
		{
			...
			if(k==1)	
			{
				document.getElementById("dealer_demo").innerHTML=dealer_hand_value+"/"+dealer_hand_value2;
				dhita=1;		
			}
			else{
				document.getElementById("dealer_demo").innerHTML=dealer_hand_value;
			}	
			if(l==1)
				{
					dhitf_for_card1=1;
			...
			if(c==1||g==1)
			{
				document.getElementById("player_demo").innerHTML=player_hand_value+"/"+player_hand_value2;
				phita=1;
				...
				document.getElementById("player_demo").innerHTML=player_hand_value;
			}
	...
	if(person=='player')
		{
			...
			document.getElementById("bottom").innerHTML="Player has won via BLACKJACK!";
			button_activator();
		}
}
...
function blackjack_checking_for_dealer()
{
	...
	{
		document.getElementById("dealer_demo").innerHTML=player_hand_value;
		button_activator();
	}
	return 1;
}
...
function push()
{
	wallet+=total_bet;
	total_bet=0;
	document.getElementById("bottom").innerHTML="It is a damn push i.e. no wins no losses!"
	button_activator();
	//Implement rebetting or replaying!
}
...
function stand()
{
	...
	while(dealer_hand_value<17||dealer_hand_value2<17)
		{
			dealer_avoiding_soft_17();
		}
	pay_offs();
}
	...
	if(dealer_hand_value>21)
	...
	else if(k==1||dhita==1)
		{
			...
			if(dealer_hand_value2>21)
				{
					document.getElementById("dealer_demo").innerHTML=dealer_hand_value+"/"+dealer_hand_value2+" -> Out of bounds!";
				}
			else{
					document.getElementById("dealer_demo").innerHTML=dealer_hand_value+"/"+dealer_hand_value2;
			}
	...
	if(final_player_hand>final_dealer_hand)
		{
			...
			document.getElementById("demo_for_bet_amount_and_wallet_amount").innerHTML="Congratulations! Wallet occupies "+wallet+"!";
			document.getElementById("bottom").innerHTML="It is a win for the player!"
			button_activator();
		}
	else if(final_dealer_hand>final_player_hand)
		{
			document.getElementById("demo_for_bet_amount_and_wallet_amount").innerHTML="You have lost! Wallet occupies "+wallet+"!";
			document.getElementById("bottom").innerHTML="It is a loss for the player and a win for the casino!";
			button_activator();
		}
	else{
		push();
...
function deal_and_double_event_activator()
{
	...
	document.getElementById("deal").onclick = function () {deal_card_displayer();double_deactivator();hit_and_stand_activator();};
}
	...
	if(wallet>initial_amount)
		{
			...
			window.open('images/gifs/next_time.gif','my_window','height=400,width=400,top=400,left=400');
	}
	document.getElementById("finally_leaving").href="front_face.html";
}